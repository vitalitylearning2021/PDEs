...
template <class T>
__global__ void reverseArrayKernel(const T* __restrict__ d_in, T* __restrict__ d_out, const int Npixels, const T a)
{
	...
	if (tid < Npixels) s_data[BLOCKSIZE_REVERSE - (id + 1)] = a * d_in[tid];

	// --- Block until all threads in the block have written their data to shared memory
	__syncthreads();

	// --- Write the data from shared memory in forward order
	if ((Npixels - offset + id) >= 0) d_out[Npixels - offset + id] = s_data[threadIdx.x];
}
...
/************************/
template <class T>
void reverseArray(const T* __restrict__ d_in, T* __restrict__ d_out, const int Npixels, const T a) {

	reverseArrayKernel << <iDivUp(Npixels, BLOCKSIZE_REVERSE), BLOCKSIZE_REVERSE, BLOCKSIZE_REVERSE * sizeof(T) >> > (d_in, d_out, Npixels, a);
#ifdef DEBUG
...
/*******************************************************/
//template <class T>
//thrust::pair<T *,T *> Cartesian2Polar(const T * __restrict__ d_x, const T * __restrict__ d_y, const int Npixels, const T a) {
//
//	T *d_rho;	gpuErrchk(cudaMalloc((void**)&d_rho,   Npixels * sizeof(T)));
//	T *d_theta; gpuErrchk(cudaMalloc((void**)&d_theta, Npixels * sizeof(T)));
//
//	Cartesian2PolarKernel<<<iDivUp(Npixels, BLOCKSIZE_CART2POL), BLOCKSIZE_CART2POL>>>(d_x, d_y, d_rho, d_theta, Npixels, a);
//#ifdef DEBUG
//	gpuErrchk(cudaPeekAtLastError());
//	gpuErrchk(cudaDeviceSynchronize());
//#endif
...
/*******************************************************/
//template <class T>
//thrust::pair<T *,T *> h_Cartesian2Polar(const T * __restrict__ h_x, const T * __restrict__ h_y, const int Npixels, const T a) {
//
//	T *h_rho	= (T *)malloc(Npixels * sizeof(T));
//	T *h_theta	= (T *)malloc(Npixels * sizeof(T));
//
...
/*******************************/
template<class T>
T h_l2_norm(T* v1, T* v2, const int Npixels) {
...
template<class T>
__global__ void vectorAddConstantKernel(T* __restrict__ d_in, const T scalar, const int Npixels) {

	const int tid = threadIdx.x + blockIdx.x * blockDim.x;

	if (tid < Npixels) d_in[tid] += scalar;

}

template<class T>
void vectorAddConstant(T* __restrict__ d_in, const T scalar, const int Npixels) {

	vectorAddConstantKernel << <iDivUp(Npixels, BLOCKSIZE_VECTORADDCONSTANT), BLOCKSIZE_VECTORADDCONSTANT >> > (d_in, scalar, Npixels);
...
template<class T>
__global__ void vectorMulConstantKernel(T* __restrict__ d_in, const T scalar, const int Npixels) {
...
template<class T>
void vectorMulConstant(T* __restrict__ d_in, const T scalar, const int Npixels) {

	vectorMulConstantKernel << <iDivUp(Npixels, BLOCKSIZE_VECTORMULCONSTANT), BLOCKSIZE_VECTORMULCONSTANT >> > (d_in, scalar, Npixels);
...
/*****************************************/
template<class T>
void h_vectorMulConstant(T* __restrict__ h_in, const T scalar, const int Npixels) {

	for (int i = 0; i < Npixels; i++) h_in[i] *= scalar;
