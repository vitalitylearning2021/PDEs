...
/***********************************************/
template <class T>
void saveGPUrealtxt(const T* d_in, const char* filename, const int Mpixels) {

	T* h_in = (T*)malloc(Mpixels * sizeof(T));

	gpuErrchk(cudaMemcpy(h_in, d_in, Mpixels * sizeof(T), cudaMemcpyDeviceToHost));

	std::ofstream outfile;
	outfile.open(filename);
	for (int i = 0; i < Mpixels; i++) outfile << std::setprecision(prec_save) << h_in[i] << "\n";
	outfile.close();
...
/***********************************************/
template <class T>
void saveCPUrealtxt(const T* h_in, const char* filename, const int Mpixels) {
...
/**************************************************/
template <class T>
void saveGPUcomplextxt(const T* d_in, const char* filename, const int Mpixels) {
...
void saveGPUcomplextxt(const float2* d_in, const char* filename, const int Mpixels) {

	float2* h_in = (float2*)malloc(Mpixels * sizeof(float2));

	gpuErrchk(cudaMemcpy(h_in, d_in, Mpixels * sizeof(float2), cudaMemcpyDeviceToHost));
...
void saveGPUcomplextxt(const double2* d_in, const char* filename, const int Mpixels) {

	double2* h_in = (double2*)malloc(Mpixels * sizeof(double2));

	gpuErrchk(cudaMemcpy(h_in, d_in, Mpixels * sizeof(double2), cudaMemcpyDeviceToHost));
...
template <class T>
T* loadCPUrealtxt(const char* filename, T* __restrict__ h_out, const int Mpixels) {

	h_out = (T*)malloc(Mpixels * sizeof(T));

	std::ifstream infile;
	infile.open(filename);
	for (int i = 0; i < Mpixels; i++) infile >> h_out[i];
	infile.close();
...
// --- Load individual real matrix from txt file
template <class T>
T* loadGPUrealtxt(const char* filename, T* __restrict__ d_out, const int Mpixels) {

	T* h_out = (T*)malloc(Mpixels * sizeof(T));
	//T *d_out;	gpuErrchk(cudaMalloc(&d_out, Mpixels * sizeof(T)));
	gpuErrchk(cudaMalloc((void**)&d_out, Mpixels * sizeof(T)));
	...
	gpuErrchk(cudaMemcpy(d_out, h_out, Mpixels * sizeof(T), cudaMemcpyHostToDevice));
